...
//    
    ...
    public static Attribute parseAttributePart(String attribute) {
        ...
        final String key, column, table;
        table = stripQuotes(m.group(1));
        column = stripQuotes(m.group(2));
        key = m.group(3) != null ? stripQuotes(m.group(3)) : column;
        return new Attribute(key, table, column, key);
    }
    
    private static final Pattern FROM_PART_PATTERN = PartPattern(
            "(.*?)(IDENT)");
    
    public static Table parseFromPart(String from) {
        ...
        if (m.group(1).isEmpty()) {
            table = QlCode.id(m.group(2));
        } else {
            table = QlCode.ql(m.group(1));
        }
        return new Table(key, table, key);
    }
...
//    
    ...
    private static String QuotedStringPattern(String q) {
    ...
    public static class Attribute {
        ...
        public Attribute(String key, String table, String column, String name) {
            this.key = key;
            this.table = table;
            this.column = column;
            this.name = name;
    ...
    public static class Table implements QlCode {
        ...
        public Table(String key, QlCode declaration, String name) {
            this.key = key;
