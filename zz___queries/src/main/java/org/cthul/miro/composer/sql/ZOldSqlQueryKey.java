...
public enum ZOldSqlQueryKey {
    
    /** Includes all parts flagged as `always`.
     * */
    PUT_ALWAYS,
...
//    INCLUDE_GENERATED_KEYS,
    ...
    public static ZOldSqlQueryKey asDataQueryKey(Object o) {
        if (o instanceof ZOldSqlQueryKey) {
            return (ZOldSqlQueryKey) o;
        }
        if (o instanceof String) {
            ZOldSqlQueryKey k = KEY_MAP.get((String) o);
            if (k != null) return k;
        }
        return UNKOWN;
    }

    private ZOldSqlQueryKey() {
    }

    private static final Map<String, ZOldSqlQueryKey> KEY_MAP = new HashMap<>();
