...
import org.cthul.miro.graph.base.EntityKeyAdapter;
import org.cthul.miro.graph.base.EntityNodeType;
import org.cthul.miro.graph.NodeSelector;
import org.cthul.miro.db.MiResultSet;
import org.cthul.miro.db.MiException;
import org.cthul.miro.db.jdbc.HsqlConnection;
...
import org.cthul.miro.graph.Schema;
...
public class ResultsTest {
    ...
    private final MiConnection connection = new HsqlConnection(TestDB::getConnection);
    
    private final Schema schema = Schema.build()
    ...
    static class PersonEntity extends ResultReadingEntityType<Person> {
        ...
        protected int[] findColumns(MiResultSet rs) throws MiException {
            return AttributeMapping.findAllColumns(rs, "id");
        }
    ...
    static class PersonNode extends EntityNodeType<Person> implements EntityKeyAdapter<Person> {
        ...
        @Override
        public EntityConfiguration<Person> newAttributeSetter(GraphApi graph, List<String> attributes) throws MiException {
            EntityConfiguration<Person> config = PERSON_FIELDS;
            List<String> addressAttributes = new ArrayList<>();
            attributes.stream().filter(a -> a.startsWith("address.")).forEach(a -> {
        ...
        @Override
        protected AttributeQuery<Person> newAttributeQuery(GraphApi graph, List<String> attributes) throws MiException {
            return newBatchLoader(graph, attributes, (cnn, keys) -> {
                    ...
                    return query.append(")").execute();                
            });
        }
        
        @Override
        protected EntityKeyAdapter<Person> getKeyAdapter() {
            return this;
        }
        ...
        public KeyReader newKeyReader(MiResultSet rs) throws MiException {
            return EntityKeyAdapter.newKeyReader(rs, "id");
        }
    }
    
    static class AddressNode extends EntityNodeType<Address> implements EntityKeyAdapter<Address> {
        ...
        @Override
        protected EntityKeyAdapter<Address> getKeyAdapter() {
            return this;
        }

        @Override
        public EntityConfiguration<Address> newAttributeSetter(GraphApi graph, List<String> attributes) throws MiException {
            return ADDRESS_FIELDS.and(new AddressPersonLink(graph, Arrays.asList("*")));
        }

        @Override
        protected AttributeQuery<Address> newAttributeQuery(GraphApi graph, List<String> attributes) throws MiException {
            return newBatchLoader(graph, attributes, (cnn, keys) -> {
                    MiQueryString query = cnn.newQuery()
