...
//        insertPerson(3, "Bob", "Brown", 1);
    ...
    private MiQueryBuilder query1() {
        return connection.newQuery().clause(MiDBString.TYPE, str -> str.append(
    ...
    private MiQueryBuilder query4() {
        MiQueryBuilder query = connection.newQuery();
        query.begin(QlBuilder.TYPE).append(
                "SELECT p.id, p.first_name, p.last_name, "
                + "a.id AS address_id, a.street AS address_street, a.city AS address_city, "
    ...
    static class AddressNode extends AbstractEntityType<Address> {
        ...
        protected MiResultSet queryAttributes(MiConnection cnn, List<Object[]> keys, List<?> attributes) throws MiException {
            MiQueryBuilder query = cnn.newQuery();
            QlBuilder<?> ql = query.begin(QlBuilder.TYPE).append(
            ...
            ql.append(") "
                    + "ORDER BY a.id");
            keys.forEach(o -> ql.pushArgument(o[0]));
            return query.execute();
        }
